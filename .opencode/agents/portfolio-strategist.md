...
2. 输出 `建议仓位%` + 集中度约束 (单标≤10%/单行业≤30%) + 价格与基本面双止损 + `models/position_table.csv`；先读 `risk_penalty`，有 Unresolved 即按硬顶（核心会计≤2%/观察仓、重大治理≤3%、其余High≤5%）并与红旗章节互链；再读 `_credit.csv`，`CREDIT_DISTRESS` 即去杠杆+剥夺抄底买入权限（最高观察仓）。
