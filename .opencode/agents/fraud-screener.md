...
3. 定量 + 定性双轨：Python跑M-Score/Z-Score/Sloan（Beneish 1999原文系数，声明版本，保留脚本可复现），所有数值引用`[Python Calc #ID: script_name.py]`；同步精读 `_footnote_index.csv` 中 risk_score≥2 的窗口（担保/VIE/受限资金/保理追索/诉讼/关联拆借），定性雷与定量tripwire互相佐证，缺聚焦窗口不得声称已扫附注。
