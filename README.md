...
| L1 Evidence | **Filing-Collector** (CNINFO/HKEX/EDGAR routing + 24-month regulatory-enquiry sweep), **Financial Researcher** (XBRL-first parsing + enquiry letters at footnote priority), **Evidence-Locker** (sole `_bibliography.csv` writer) |
| L2 Analysis | **Fraud Screener** (M-Score/Z-Score/Sloan + governance red-flags + footnote focus windows with cross-ref expansion + enquiry letters, main-chain only), **Black Account Checker** (salted-PII private transaction forensics with treasury-pool filtering, on-demand sidecar), **Financial Analyst** (multi-doc reconciliation PIT as-reported/restated, Pre-Valuation initiator), **Industry-Peer Analyst** (peer benchmarking + macro + ESG screening), **Sentiment-Event Analyst** (event calendar, risk-only sidecar) |
| L3 Valuation | **Valuation Expert** (dispatcher-selected engine: DCF/Multiples/PB-ROE/DDM/rNPV + SOTP for conglomerates + distressed fallback + SBC/lease adjustments + Unresolved Haircut), **Scenario-Sensitivity Analyst** (Bull/Base/Bear + clamped WACC×g matrix), **Portfolio Strategist** (penalty-aware sizing/concentration/stops with position hard caps) |
| L4 Risk | **Adversarial Skeptic** (polarized short-seller CIO, owns Challenge Log with max-2-round fuse → tiered Unresolved), **Compliance Checker** (trilingual wording review, T3), **Judge-QA** (3-way reconciliation + penalty/price/position/flag consistency, heterogeneous T3 model) |
...
- **Enquiry Parity**: 24-month regulatory enquiry letters + replies rank with footnotes in focus windows; screeners must read them before concluding, never claim "no regulatory challenge" when uncovered.
- **Treasury-Aware Forensics**: cash-pooling sweeps fingerprinted (memo + timing + offset, ≥2 to tag `[TREASURY_POOL]`) and excluded from malicious fast-in-out/circular counts with appendix disclosure; disguised sweeps escalate to High.
- **Polarized Red Team**: skeptic runs as aggressive short-seller CIO (≥3 fatal flaws / −30% price target KPI); evidence-free approval counts as sycophancy and is sent back.
